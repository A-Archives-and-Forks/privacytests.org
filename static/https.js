/* global postDataAndCarryOn, showError */
...
const insecureSubresourceTest = async (tag, fileName) => {
  const fileTypeNames = { img: 'image', script: 'script' };
  ...
  const upgradableEvent = await loadSubresource(tag, `http://upgradable.privacytests2.org/content/${fileName}`);
  const insecureEvent = await loadSubresource(tag, `http://insecure.privacytests2.org/content/${fileName}`);
  const passed = insecureEvent.type === 'error' || insecureEvent.message === 'timeout';
  const putativeUpgradeHandling = upgradableEvent.type === 'load' ? 'upgraded' : 'blocked';
  const result = passed ? putativeUpgradeHandling : 'loaded insecurely';
